...
ENV GOWORK=off
RUN cd /app/transports && go build \
    -ldflags="-w -s -extldflags '-static' -X main.Version=v$(cat /app/transports/version)" \
    -a -trimpath \
    -tags "sqlite_static" \
    -o /app/main \
    ./bifrost-http
...
COPY --from=builder /app/transports/docker-entrypoint.sh /app/docker-entrypoint.sh
