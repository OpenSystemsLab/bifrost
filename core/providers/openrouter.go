...
func (provider *OpenRouterProvider) TextCompletion(ctx context.Context, key schemas.Key, input *schemas.BifrostRequest) (*schemas.BifrostResponse, *schemas.BifrostError) {
	...
	openRouterReq := openrouter.ConvertTextRequestToOpenRouter(input)
	if openRouterReq == nil {
		return nil, newBifrostOperationError(schemas.ErrProviderJSONMarshaling, nil, schemas.OpenRouter)
	}

	jsonBody, err := sonic.Marshal(openRouterReq)
	if err != nil {
	...
	bifrostResponse := openrouter.ConvertOpenRouterTextResponseToBifrost(response)
