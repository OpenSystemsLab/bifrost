...
func (provider *AzureProvider) TextCompletion(ctx context.Context, key schemas.Key, input *schemas.BifrostRequest) (*schemas.BifrostResponse, *schemas.BifrostError) {
	...
	openaiReq := openai.ConvertTextRequestToOpenAI(input)
	if openaiReq == nil {
		return nil, newConfigurationError("failed to convert text request", schemas.Azure)
	}

	responseBody, err := provider.completeRequest(ctx, openaiReq, "completions", key, input.Model)
	if err != nil {
		return nil, err
	}
	...
	bifrostResponse := openai.ConvertOpenAITextResponseToBifrost(response, input.Model, schemas.Azure)
...
func (provider *AzureProvider) Responses(ctx context.Context, key schemas.Key, input *schemas.BifrostRequest) (*schemas.BifrostResponse, *schemas.BifrostError) {
	...
	responseBody, err := provider.completeRequest(ctx, requestBody, "responses", key, input.Model)
	if err != nil {
		return nil, err
	}
